...
const EstadisticasGirasRealizadas = () => {
  const navigate = useNavigate();

  const { girasDone, allGiras, setGiras } = girasListForAdmin();
  ...
  useEffect(() => {
    // if (girasDone.length == 0) return;
    const f = async () => {
      const idsOfGira = girasDone.map((gira) => gira.currentId);
      ...
      setStatistics(girasDone, reservationsForStatistics);
    };
    f();
  }, [girasDone]);
  ...
  return (
    <>
      <Headers text="Estadisticas giras realizadas" link={-1} />
      <div className="my-4">
        {/* {giras.map((gira) => (
          <ListGiras
            key={gira.currentId}
...
export default EstadisticasGirasRealizadas;
